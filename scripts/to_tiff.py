import numpy as np
import cv2
folderPath = "/home/link/Projects/curly_slam/data/tartanair/scenes/seasidetown/Easy/P001"
...
def readimage(imageIndex):
    imageName = '0'*(6 - len(str(imageIndex))) + \
        str(imageIndex) + "_left_depth.npy"
    ...
    cv2.imwrite(savePath + '0'*(6 - len(str(imageIndex))) +
                str(imageIndex) + "_left_depth.tiff", depth)
    # imageIndex += 1
